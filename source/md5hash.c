...
void md5hash(unsigned char pass[PASSL],unsigned char key[KEYSL], unsigned int *res);
...
void md5hash(unsigned char pass[PASSL], unsigned char key[KEYSL], unsigned int *res){
    unsigned int bits[KEYSL], keys[4], trans = 0, f = 0,g = 0,i = 0, x = 0, k[PASSL], s[PASSL] = {7,12,17,22,7,12,17,22,7,12,17,22,5,9,14,20,5,9,14,20,5,9,14,20,4,11,16,23,4,11,16,23,4,11,16,23,6,10,15,21,6,10,15,21,6,10,15,21};
    for(i = 0; i < PASSL; i++){
        if(pass[i] == 10)break;
    }
    for(x = 0; x < (PASSL-i) ; x++){ 
